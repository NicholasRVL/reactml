...
export default function PredictionPanel({ result }) {
  ...
  const shinyForms = forms.filter(f => f.is_shiny === true)
          ...
          <div className="p-4 h-100 shadow-sm border-0">
            ...
            {Object.keys(stats).length > 0 && (
              <div style={{ background: "#c2c2c263", borderRadius: "20px", padding: "20px", color: "black", marginTop: "20px" }}>
                ...
                {Object.entries(stats).map(([key, value]) => {
                  ...
                  return (
                    <div key={key} style={{ marginBottom: "12px" }}>
                      ...
                      <div style={{ background: "#333", borderRadius: "10px", overflow: "hidden", height: "10px" }}>
                        <div
                          style={{
                ...
                {stats.total && (
                  <div style={{ marginTop: "12px", textAlign: "right", fontWeight: "bold", opacity: 0.85 }}>
                    Total: {stats.total}
                  </div>
                )}
              </div>
            ...
            <div className="d-flex gap-2 flex-wrap">
              {predictions.map((p, i) => (
                <span key={i} className={`badge ${i === 0 ? 'bg-success shadow-sm' : 'bg-white text-dark border'}`}>
                  {p.name} · {p.confidence}%
                </span>
              ))}
            </div>
                ...
                {result.evolution_chain.map((p, i) => (
                  <div key={i} className="text-center text-light">
                    ...
                    <img
                      ...
                      alt={p.name}
                      width={85}
                      style={{ filter: 'drop-shadow(0 10px 14px rgba(0,0,0,.15))' }}
                    />
                    <div className="fw-bold text-capitalize mt-2 small text-light">{p.name}</div>
                  </div>
                ))}
              </div>
            ) : (
                ...
                <div className="d-flex gap-4 overflow-auto pb-2">
                  {megaForms.map((m, i) => (
                    <div key={i} className="text-center text-light" style={{ minWidth: 90 }}>
                      <img src={m.image} alt={m.name} width={80} style={{ filter: 'drop-shadow(0 0 10px rgba(255,0,0,0.2))' }} />
            ...
            {shinyForms.length > 0 && (
              <>
                ...
                <div className="d-flex gap-4 overflow-auto pb-2">
                  {shinyForms.map((s, i) => (
                    <div key={i} className="text-center text-light" style={{ minWidth: 90 }}>
                      <img src={s.image} alt={s.name} width={80} style={{ filter: 'drop-shadow(0 0 15px rgba(255,215,0,0.4))' }} />
